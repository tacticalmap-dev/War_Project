...
public final class NodeLJYSCaptureHudOverlay
{
    ...
    private NodeLJYSCaptureHudOverlay()
    {
    }
    ...
    {
        event.registerAbove(VanillaGuiOverlay.HOTBAR.id(), "war_project_capture_progress", NodeLJYSCaptureHudOverlay::render);
    }
    ...
    private static void renderCaptureProgress(GuiGraphics graphics, Minecraft minecraft, WarProjectNetwork.CaptureProgressPacket snapshot, int screenWidth, int screenHeight)
    {
        ...
        int ringColor = NodeLJYSCaptureHudState.activeColor(snapshot);
        float progress = NodeLJYSCaptureHudState.smoothedProgress();
        ...
        String percentText = NodeLJYSCaptureHudState.progressPercent(snapshot) + "%";
        graphics.drawCenteredString(minecraft.font, percentText, centerX, centerY + (RING_SIZE / 2) + 3, 0xDDEDEDED);
    }

    private static void renderCaptureNotice(GuiGraphics graphics, Minecraft minecraft, NodeLJYSCaptureNoticeHudState.CaptureNotice notice, int screenWidth, int screenHeight)
    {
        int centerX = screenWidth / 2;
    ...
    {
        String localFaction = NodeLJYSCaptureHudState.localFactionId();
