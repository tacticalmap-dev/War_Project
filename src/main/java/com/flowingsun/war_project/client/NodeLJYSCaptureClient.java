...
public final class NodeLJYSCaptureClient {
    private static final int INTENT_INTERVAL_TICKS = 20;

    private static int tickCounter;

    private NodeLJYSCaptureClient() {
    }
    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        NodeLJYSCaptureHudState.tick();
        NodeLJYSCaptureNoticeHudState.tick();

        ChunkPos chunk = player.chunkPosition();
        Optional<ClientMapState.ClientNode> node = ClientMapState.nodeAt(chunk.x, chunk.z);
        NodeLJYSCaptureHudState.setCurrentNode(node.map(ClientMapState.ClientNode::id));
    ...
    public static void onLogout(ClientPlayerNetworkEvent.LoggingOut event) {
        tickCounter = 0;
        NodeLJYSCaptureHudState.reset();
        NodeLJYSCaptureNoticeHudState.reset();
