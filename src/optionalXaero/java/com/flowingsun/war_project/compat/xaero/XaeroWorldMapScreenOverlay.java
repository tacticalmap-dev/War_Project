...
public final class XaeroWorldMapScreenOverlay {
    ...
    private static void drawWarzoneEdges(GuiGraphics graphics, MapProjection projection, int screenWidth, int screenHeight) {
        EdgeCollector collector = new EdgeCollector(dashPattern(projection));
        collectWarzoneEdges(collector, projection, screenWidth, screenHeight);
        collector.draw(graphics);
    }

    private static void drawNodeEdges(GuiGraphics graphics, MapProjection projection, int screenWidth, int screenHeight) {
        EdgeCollector collector = new EdgeCollector(dashPattern(projection));
    ...
    private static void collectChunkEdges(EdgeCollector collector, Rect rect, int color, boolean dashed, boolean west, boolean east, boolean north, boolean south, int priority) {
        if (west) {
            collector.addVertical(rect.x1(), rect.y1(), rect.y2(), color, dashed, priority);
        }
        if (east) {
            collector.addVertical(rect.x2() - 1, rect.y1(), rect.y2(), color, dashed, priority);
        }
        if (north) {
            collector.addHorizontal(rect.y1(), rect.x1(), rect.x2(), color, dashed, priority);
        }
        if (south) {
            collector.addHorizontal(rect.y2() - 1, rect.x1(), rect.x2(), color, dashed, priority);
        }
    }

    private static void drawVerticalLineAvoiding(GuiGraphics graphics, int x, int y1, int y2, int color, boolean dashed, DashPattern dashPattern, Set<Long> reservedPixels) {
        int runStart = Integer.MIN_VALUE;
        Set<Long> newlyReserved = new HashSet<>();
        for (int y = y1; y < y2; y++) {
            long pixel = packedPixel(x, y);
            ...
            newlyReserved.add(pixel);
            boolean drawPixel = !dashed || y2 - y1 < dashPattern.minimumDashedLength() || dashPattern.on(y);
            if (drawPixel) {
                if (runStart == Integer.MIN_VALUE) {
                    runStart = y;
                }
        ...
        reservedPixels.addAll(newlyReserved);
    }

    private static void drawHorizontalLineAvoiding(GuiGraphics graphics, int x1, int x2, int y, int color, boolean dashed, DashPattern dashPattern, Set<Long> reservedPixels) {
        int runStart = Integer.MIN_VALUE;
        for (int x = x1; x < x2; x++) {
            long pixel = packedPixel(x, y);
            if (reservedPixels.contains(pixel)) {
            ...
            boolean drawPixel = !dashed || x2 - x1 < dashPattern.minimumDashedLength() || dashPattern.on(x);
            if (drawPixel) {
                if (runStart == Integer.MIN_VALUE) {
                    runStart = x;
                }
        ...
        for (int x = x1; x < x2; x++) {
            reservedPixels.add(packedPixel(x, y));
        }
    }

    private static DashPattern dashPattern(MapProjection projection) {
        Rect sample = chunkRect(projection, 0, 0, 1, 1);
        int chunkPixels = Math.max(1, Math.max(sample.x2() - sample.x1(), sample.y2() - sample.y1()));
    ...
    private static final class EdgeCollector {
        ...
        private EdgeCollector(DashPattern dashPattern) {
            this.dashPattern = dashPattern;
        }

        void addVertical(int x, int y1, int y2, int color, boolean dashed, int priority) {
            add(new EdgeKey(true, x, color, dashed, priority), y1, y2);
        }

        void addHorizontal(int y, int x1, int x2, int color, boolean dashed, int priority) {
            add(new EdgeKey(false, y, color, dashed, priority), x1, x2);
        }
        ...
        void draw(GuiGraphics graphics) {
            ...
            entries.sort(Comparator
                    ...
                    .thenComparingInt(entry -> entry.getKey().fixed()));
        ...
        private void drawMerged(GuiGraphics graphics, EdgeKey key, int start, int end, Set<Long> reservedPixels) {
            if (key.vertical()) {
                drawVerticalLineAvoiding(graphics, key.fixed(), start, end, key.color(), key.dashed(), dashPattern, reservedPixels);
            } else {
                drawHorizontalLineAvoiding(graphics, start, end, key.fixed(), key.color(), key.dashed(), dashPattern, reservedPixels);
            }
        }
    }

    private record EdgeKey(boolean vertical, int fixed, int color, boolean dashed, int priority) {
    }
    ...
    private record DashPattern(int dash, int gap) {
        boolean on(int screenCoordinate) {
            return Math.floorMod(screenCoordinate, dash + gap) < dash;
        }
